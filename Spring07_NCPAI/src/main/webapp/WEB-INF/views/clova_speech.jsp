...
<script type="text/javascript">
	$(()=>{
		$('#frm').submit((e)=>{
			e.preventDefault(); // submit 되지 못하게 함
			let file=$('#mp3file')[0];
			if(file.files.length==0){
				alert('mp3파일을 첨부하세요')
				return;
			}
		// 파일 업로드 시 FormData 객체 사용
		let form=$('#frm')[0];
		...
		$.ajax({
			...
			dataType:'json',
			cache: false,
			processData: false, // true => get방식으로 데이터 전달, false => post 방식
			contentType: false, // false로 지정해야 enctype="multipart/form-data"로 전송 가능함
		})
		.done((res)=>{
			//alert(JSON.stringify(res));
			//alert(res.result.text); => string타입으로 옴
