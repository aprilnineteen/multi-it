...
public class NCP03VoiceController {
	...
	public ModelMap voiceToText(String content, HttpSession ses) {
		log.info("content={}", content);
		String upDir = ses.getServletContext().getRealPath("/upload");
		log.info("upDir={}",upDir);
		String fileName ="mp3파일명 들어올예정";
		//-------------------------------------
	...
	private String getNaverVoice(String content, String upDir) {
		 try {
	            ...
	            // post request
	            String postParams = "speaker=vmikyeong&volume=0&speed=0&pitch=0&format=mp3&text=" + text+"&emotion=3";
	            ...
	            if(responseCode==200) { // 정상 호출
	                ...
	                f.createNewFile(); // 임시파일명의 파일 생성. 아직 내용이 들어 있지는 않음
	                OutputStream outputStream = new FileOutputStream(f);
	                while ((read =is.read(bytes)) != -1) { // 네이버가 보내온 음성파일을 bytes 배열에 넣고
	                    outputStream.write(bytes, 0, read); // tempname.mp3 파일에 bytes 배열에 담긴 데이터 출력
	                    //was서버 upload 디렉토리에 파일 형태로 저장된다
	                }
	                is.close();
	                return f.getName(); // 생성된 mp3 파일명 반환
