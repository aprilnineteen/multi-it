...
public class Ncp01PapagoController {
	...
	//apiKey.properties파일에서 papago_clientId에 해당하는 값을 찾아 주입해준다
	...
	@ResponseBody //네이버로 부터 받은 json문자열을 응답데이터로 보내자
	public String papago_translate(String str){
		...
		try {
			...
			br.close();
			String responseData=buf.toString();//응답 데이터(json형태로 온다)
			return responseData;
		}catch(Exception e) {
			e.printStackTrace();
