...
<title>피자 주문서</title>
	...
	<script>
		...
		const getUserInfo=function(){
			...
			//[2] 요청을 보내기전에 onreadystatechange 속성값에 콜백함수 지정
		...
		//			  2. xml 형식이면 responseXML
		//			  3. json 형식이면	 responseText로 받은 뒤 JSON.parse()함수를 이용해서 문자열을 json객체로 변환해야 한다
		...
		const updatePage2=function(){
			if(req.readyState==4 && req.status==200){
				...
				//xml로 받을 경우===>XMLDocument객체로 응답옴==>이 문서를 파싱하여 필요한 데이터를 추출한다==>jquery의 find()함수 사용
				//alert(res);
				...
				if(parseInt(num)==0){//비회원일 경우
					...
					//phone태그의 type속성을 찾아 넣으세요 attr(), prop()
		...
		//응답데이터를 1. text형태(html)   2. xml  3. json
		//1.text로 받을 경우
