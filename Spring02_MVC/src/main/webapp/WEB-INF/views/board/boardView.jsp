...
<style>
	...
	table{
		...
		table-layout: fixed; /*테이블 너비 고정시킴 */
		overflow: auto;
	}
	...
	<table class="table table-bordered">
		...
		<tr>
			...
			<c:if test="${vo.filename ne null}">
				...
				<!-- 첨부파일명을 모두 소문자로 변환 후 fname이라는 변수에 할당 -->
				<c:set var="fname" value="${fn:toLowerCase(vo.filename) }"/>
				<%-- <h3>${fname}</h3> --%>
				<c:if test="${fn:endsWith(fname, '.png') or fn:endsWith(fname, 'jpg') or
				fn:endsWith(fname, 'gif')}">
		...
		<tr>
			<td width="20%" class="m1"><b>글 제목</b></td>
			<td colspan="3">${vo.subject }</td>
		</tr>
		<tr>
			<td width="20%" class="m1"><b>글 내용</b></td>
			<td colspan="3">
		...
		<tr>
			...
			<c:if test="${loginUser.userid eq vo.userid }">
				...
				<a href="#"	onclick="goEdit()">글 수정</a>| 
				<a href="#" onclick="goDel()">글 삭제</a> |
			</c:if>
