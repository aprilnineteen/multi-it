			...
			<div class="modal-dialog modal-xl">
				<div class="modal-content">
					...
					<div class="modal-body">
						<div class="row">
							<!-- 리뷰 쓰기 ------------------------>
							<!-- xl,lg,md(desktop), sm(tablet), xs(mobile) -->
							<div class="col-md-6 col-sm-12 col-xs-12">
								<!--글쓰기 폼 --------------------------- -->
								<form name="rf" id="rf" method="post"
									enctype="multipart/form-data">
									<!-- hidden data ================== -->
									<input type="hidden" name="userid" id="userid" value="nanana">
									<input type="hidden" name="pnum" id="pnum" value="${param.pnum }"> <input
										type="text" name="no" id="no" value="0">
									<!-- 글수정시 사용 -->
							...
							<div class="col-md-6 col-sm-12 col-xs-12">
								<ul id="reviewList">
								<!--  
									<li><img src="resources/" class="img img-thumbnail">
									</li>
									<li><span class="text-danger">★★★★★</span><br> <span>[title]</span><br>
										상품평이 어쩌고 저쩌고</li>
									<li></li>
									-->
...
<script>
	$(function(){
		getReviewAll('${param.pnum}');//특정 상품의 리뷰 게시글 목록 가져오기
	}) //$()end----------------
	  ...
	  function send(){
			...
			//alert(btnTxt);
			//[1] 파일을 업로드하지 않을 경우 input값을 파라미터 문자열로 만들어 전송해야 함 title=aaa&content=bbb&score=5
			let paramData=$('#rf').serialize();
			//alert(paramData);
			//[2] 파일을 업로드할 경우 ==> FormData 객체를 이용
			let form=$('#rf')[0] //form 객체
			//alert(form)
			let formData = new FormData(form); //파일 업로드 시 FormData 전송
			//firnData.append("name", "sam") //key, value
			if(btnTxt=='글쓰기'){
				
				//writeReview(paramData);//파일업로드 하지 않을 경우
				writeReviewAndFile(formData); // 파일 업로드 시 FormData  넘기자
			}else if(btnTxt=='글수정'){
			...
			function writeReviewAndFile(formData){
				$.ajax({
					...
					processData:false, //일반적으로 서버에 전달하는 데이터는 query string 형태인데, false를 주면 querty string을 사용한다
					cache: false,
			...
			//PUT /reviews => 수정처리 (파일 업로드할 때는 POST로 하자)
			function updateReviewAndFile(formData){
				/*let file=$('#filename')[0].value;
				formData.append("filename", file);
				alert(file)*/
				formData.append("mode", "edit"); //mode값이 edit이면 수정 처리 => 컨트롤러에서 비어 있으면 등록 처리
				$.ajax({
					type:'post',
					url:'reviews',
					data:formData,
		...
		function showList(res){//res=>배열
			...
			$(res).each(function(i, obj){
				...
				str+=`<li><img src="resources/images/noimage.png" class="img img-thumbnail"></li>`;
				}else{
				...
				<p>\${obj.content}</p>
				`;
				//글쓴이와 로그인한 사람이 일치한다면 수정, 삭제 링크
				if(obj.userid=='nanana'){
