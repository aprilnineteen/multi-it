...
public class PostsDAOImpl implements PostsDAO {
	...
	public int insertPosts(PostsVO vo) {
		...
		Query query=query(where("collectionName").is("posts"));//static import했을 경우
	...
	public List<PostsVO> listPosts() {
		...
		query.with(by(Direction.DESC,"no")); //static import한 경우
	...
	public PostsVO selectPosts(String id) {
		...
		PostsVO pvo=mongoTemplate.findOne(query(where("_id").is(id)),
				PostsVO.class, "posts");
		return pvo;
	}
