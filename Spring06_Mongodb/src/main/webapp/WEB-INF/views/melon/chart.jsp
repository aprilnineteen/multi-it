...
<script>
	...
	$(()=>{
		...
		.done((res)=>{
			...
			if(res.result==100){
				...
				$('#btn2').trigger('click'); // 크롤링 완료 후 멜론 차트 목록 버튼 클릭 이벤트 발생
			}
		})
		.fail((err)=>{
	...
	function showSingerCount(jsonArr){
		...
		data.addRow(mydata); // 그래프로 표현할 데이터를 추가
		renderChart(data); // 그래프로 그려 주는 함수 호출
		str +="</ul>";
	...
	function renderChart(data){
		 // Set chart options
        var options = {'title':'멜론 차트 가수별 노래 수',
                       'width':100%,
                       'height':600,
                       'fontSize': 11,
                       'fontName': 'Verdana'
