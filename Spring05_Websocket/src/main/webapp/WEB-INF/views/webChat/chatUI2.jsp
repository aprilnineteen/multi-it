...
<script>
	...
	function chat_connect(){
		...
		socket=new SockJS("${pageContext.request.contextPath}/chat"); //end point => "/chat"으로 설정
		
		stompClient=Stomp.over(socket); //소켓을 이용해서 stomp생성
		//stomp 이용해서 서버에 연결
		stompClient.connect({}, function(frame){
			//alert('연결됨: '+frame);
			$('#status').html(nickname+" 님이 챗 서버에 연결되었습니다");
			setConnected(true);
			$('#inputMsg').focus(); //대화 내용 입력 박스에 포커스 주기
			//서버로 메시지를 보내는 함수
	...
	function sendInput(mymsg){
		//alert(event.keyCode)
		//alert(mymsg);
		if(event.keyCode==13){//엔터 쳤을 때 서버에 메시지 전송
			//아스키코드 13 = ENTER
			if(mymsg!=''){
				sendMessage(nickname, 'all', mymsg); //서버로 메시지 전송
				$('#inputMsg').val(''); // jquery 버전
				//document.getElementById('inputMsg'),value=''; // (js 버전)
			}
		}
	}
	...
	function sendMessage(from, to, text){
		...
		let obj={
				...
				//         접두사 접미사 header  body (문자열로 직렬화)
		stompClient.send('/app/chat',{}, JSON.stringify(obj));
	}//----------------------------------------------------------
	
	function showChatMessage(obj){ // 대화 내용 출력해 주는 함수
...
<body>
	<div class="container">
		...
		<div class="alert alert-success my-4">
			<strong id="status">채팅을 연결 후 사용하세요....</strong>
		</div>
